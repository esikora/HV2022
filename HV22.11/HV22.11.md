...
Entering `http://169.254.169.254/latest/meta-data/iam/security-credentials/` into Santa's screenshot form reveals there is an IAM role `Hackvent-SecretReader-EC2-Role`.

We obtain a screenshot of the temporary security credentials by means of `http://169.254.169.254/latest/meta-data/iam/security-credentials/Hackvent-SecretReader-EC2-Role`:
...
Therein, we have to keep in mind that the credentials are temporary. We can observe that new credentials are generated every hour.
